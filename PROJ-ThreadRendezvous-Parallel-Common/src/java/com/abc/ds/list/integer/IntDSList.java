...
/**
 * A list of int's (always ordered and duplicates are allowed).
 * This {@link IntDSList} IS-A {@link IntDSSack}.
 */
public interface IntDSList extends IntDSSack {
     ...
     */
    void insertBefore(int index, int item) throws IndexOutOfBoundsException;
     ...
     */
    int firstIndexOf(int item, int fromIndex);
     ...
     * reduced to (count - 1).
     */
    int lastIndexOf(int item, int fromIndex);
     ...
     * @throws IndexOutOfBoundsException if index < 0 or index >= count()
     */
    int removeAtIndex(int index) throws IndexOutOfBoundsException;
     ...
     * {@link #isEmpty()} first to avoid this exception.
     */
    int peekFirst() throws NoSuchElementException;
     ...
     * {@link #isEmpty()} first to avoid this exception.
     */
    int peekLast() throws NoSuchElementException;

    /**
     * Changes the item stored at the specified index.
     * @throws IndexOutOfBoundsException if index < 0 or index >= count()
     */
    void replaceAtLindex(int index, int replacementItem)
     ...
     * render the iterator invalid.
     */
    IntDSIterator createReverseIterator();
}
